...
//     outright. The system file is the right place for it regardless of whether the
//     `GIT_CONFIG_*` environment would also serve — it does, measured against git 2.47.3 in
//     the Boks base image, and gitSafeDirectoryEnv in sandbox.go relies on that for the
//     direct-mode workspace. Here the statement is about a mount the whole guest shares, so
//     it belongs in the guest's own configuration rather than in one process's environment;
//     that also means the agent's own
