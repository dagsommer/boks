...
func commands() []command {
	return []command{
		...
		{name: "net", summary: "Inspect or stop the network stack serving a sandbox", run: netCommand},
		{name: "proxy", summary: "Run the host forward proxy on its own, outside any sandbox", run: proxyCommand},
		{name: "secret", summary: "Manage host-side credentials the guest never receives", run: secretCommand},
		{name: "ca", summary: "Inspect or replace the local CA used for TLS interception", run: caCommand},
