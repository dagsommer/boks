...
| Hypervisor per platform | Apple Virtualization Framework on Apple silicon; KVM on Linux | libkrun: Hypervisor.framework on macOS, KVM on Linux, and the Windows Hypervisor Platform from this project's patch series | P0 | partial | macOS path verified 2026-08-11; the WHP backend runs a Boks sandbox with policy enforced, 2026-08-14 and 2026-08-15; Linux/KVM verified 2026-08-15 but in WSL2, never on bare metal |
...
| `ls` | Lists sandboxes with status and port mappings | `boks ls`/`boks list`, `-q`, `--json` | P1 | partial | sbx's columns exactly: SANDBOX AGENT STATUS PORTS WORKSPACE. PORTS is populated, in Docker's `127.0.0.1:8080->3000/tcp` notation, read from the supervisors' state files rather than by opening a socket per sandbox — a listing must not be hangable by one wedged background process |
...
| `ls` columns | `SANDBOX  AGENT  STATUS  PORTS  WORKSPACE` | Identical, and PORTS finally has something in it: every binding a sandbox publishes, comma-separated | P1 | done |
...
| Port publishing | `--publish` at creation; `sbx ports SANDBOX --publish/--unpublish/--json` later; ignored when re-attaching | Same, all four | P1 | partial | **TCP only** — see the next row but one. A published port is a host listener on loopback spliced to a connection into the guest's virtual network; gvisor-tap-vsock's generic `Forwards` map is still asserted empty, because a per-port hole a human asked for is not the same thing as a mechanism. Publishing on a *running* sandbox needs the supervisor told something, and it was built with no API: it now has an owner-only UNIX socket in the state directory speaking three verbs (list, publish, unpublish), reachable by the sandbox's owner and structurally not by the guest — see `internal/enforce/control.go` for the argument and the two shapes that were rejected |
| Port binding defaults | Binds **loopback** by default, expanded per address family: `tcp`/`udp` bind both `127.0.0.1` and `::1`, or v4 only for a v4-only sandbox; `tcp4`/`udp4` v4 only; `tcp6`/`udp6` v6 only. Protocols: tcp, tcp4, tcp6, udp, udp4, udp6 | Same | P1 | done | Copied exactly, and pinned by a test that asserts every protocol in both expansions binds a loopback address. A boks sandbox's virtual network is IPv4-only, so the default binds `127.0.0.1` alone; an explicit `tcp6` still binds `::1` on the host and forwards over the guest's IPv4, because the host binding and the guest's address family are different questions and an OAuth callback on `[::1]` has to be reachable |
| UDP publishing | `udp`, `udp4`, `udp6` are accepted protocols | Parsed, then refused with the reason | P2 | none | **Gap.** The stack drops UDP at the link — the whitelist carries TCP, ARP and DNS to the gateway — so a datagram has no return path. Publishing UDP would mean widening that filter with a conntrack-like exception, which is a change to the stack's closed posture and not one to make as a side effect of a port flag. The grammar still accepts the spelling so that a user who types it gets the reason rather than "unknown protocol" |
| Guest binding requirement | Service must listen on the VM's external interface, not just loopback | Same constraint, and the error says so | P1 | done | Only the guest's external address is addressable across the link. A connection that finds nothing there is logged and recorded on the binding, so `boks ports` shows "the service in the sandbox must listen on the VM's external interface (bind 0.0.0.0 or ::, not only 127.0.0.1)" rather than a bare refusal |
...
| OAuth response fields / passthrough | `responseFields` overrides, `passthrough` skips masking | `responseFields` equivalent; masking on the acquisition path only, and no way to turn it off | P2 | done | `passthrough` is very likely this same distinction seen from the other side: a *relayed* response is the only kind that needs masking, and a *refresh* is composed from sentinels so there is nothing to mask. Boks masks by value rather than by field — a token echoed anywhere is replaced — and a body it cannot fully mask fails the exchange instead |
...
| OAuth acquisition in the sandbox | `sbx run claude … -- auth login`: the real client performs its own OAuth inside the sandbox and sbx ends up holding the token | `boks secret login SERVICE` arms a credential, `boks run claude -- auth login` fills it in | P0 | partial | The initial exchange is **relayed and captured** — the opposite of the refresh path, because the authorization code exists only inside the guest — and the response is rewritten to sentinels before the guest sees a byte. Verified against a local origin, with the exchange shape copied from the real Claude Code. **Not durable through a sandbox**: the network supervisor has no store passphrase, so the login lasts as long as the sandbox. Needs no port publishing — the flow is paste-a-code, confirmed on the real binary |
...
Read from real `sbx secret --help` and `sbx secret set/import --help`. **Done**, apart from the
host-side `--oauth`, which is refused on purpose; the in-sandbox acquisition it exists beside
is in §5b. What follows is what was found and what was built from it.
...
- **Two OAuth acquisition paths, and Boks now has both of the ones that matter.**
  `sbx secret set -g openai --oauth` runs the flow host-side; `sbx run claude … -- auth login`
  drives it through the agent inside the sandbox. The host-side one is refused here and stays
  refused — every flow that could acquire a token begins by identifying the program to the
  vendor with a client id issued to a registered application, and Boks is registered with none
  — and it is worth noticing that sbx's is documented "openai/global only", which is what
  holding exactly *one* registration looks like. **The in-sandbox one is not blocked by any of
  that**, because the client id belongs to the agent, and Boks was already terminating TLS for
  the endpoint it uses. So `boks secret login` arms a credential and `boks run claude -- auth
  login` fills it in, with the initial exchange relayed and captured and the response rewritten
  to sentinels; and `boks secret adopt`, which sbx does not appear to have at all, still covers
  the machine you already logged in on. See §5b and the rows below.
...
| `secret login` | Not a command; the equivalent is `sbx run claude … -- auth login` with no setup | `boks secret login SERVICE` arms the credential first | P1 | done | Boks makes the arming explicit rather than doing it for every run of an agent, because arming a credential is what decides that boks decrypts that host's traffic — the same bargain every credential rule makes, struck where the user can see it |
...
| OAuth device login | Agent login inside sandbox; session tokens stay on host | Implemented as acquisition, not as a device flow | P1 | partial | See §5b. No device flow is involved: the agent runs its own authorization-code-with-PKCE login and boks keeps the result |

### 5b. Acquisition: the login happens in the sandbox, the token does not leave the host

The gap `boks secret adopt` cannot close is a **fresh machine**, where there is no login to
read. sbx closes it with `sbx run claude … -- auth login`, and the reason that works for a
product with no Anthropic registration is that it is not the product logging in — it is the
agent, with the agent's own client id, which is legitimate because it *is* that program. Boks
does the same, and most of the machinery was already there.

**What was determined first, because it decides whether this is possible at all.** Two login
shapes are common and they behave very differently in a sandbox: paste-a-code works today,
while a localhost redirect needs the browser on the host to reach a listener in the guest —
which without port publishing never arrives. Claude Code 2.1.228 was driven headless to find
out. It uses **paste-a-code**: `redirect_uri` is a vendor-hosted page, `code=true` selects the
paste flow, and the binary contains no loopback callback at all. So this needs no ports, and
the fact that sbx *has* `sbx ports` is not the reason its version of this works. See
[verification.md](verification.md#the-real-claude-code-login-flow-observed).

**The design, in one sentence:** an armed credential — sentinels, endpoint, destinations, no
tokens — makes the proxy relay the *first* token exchange it sees for that endpoint, keep the
pair out of the response, and hand the guest the same document with sentinels in it.

The thing to keep straight is that this is the **opposite** of the refresh path, which was
built earlier and stays as it was:

| | refresh | acquisition |
|---|---|---|
| the guest's request | drained and discarded, never forwarded | relayed verbatim |
| why | Boks can compose the request itself from the stored refresh token | the authorization code and PKCE verifier came from a redirect only the guest saw |
| the response | composed by Boks from sentinels | the origin's, with the tokens replaced |
| the body | never buffered | buffered, bounded, and refused if it cannot be masked |
| how often | every time | once, and never again for that credential |

Which one applies is read from the **stored record**, never from the request: a credential
relays only while it holds no token, and the first token closes the door. A guest cannot ask
for the relay path and cannot re-open it.

Docker's v2 grammar documents `passthrough: boolean to skip sentinel masking` on its `oauth`
block. That is very likely this same distinction from the other side — a relayed response is
the only kind there is anything to mask *in* — and Boks deliberately has no equivalent switch.

**Two things this does not do**, both worth stating rather than discovering:

- **It is not durable through a sandbox.** The network supervisor never learns the store's
  passphrase, so a login it captures lasts as long as that sandbox. This is the same
  limitation `internal/secret/memory.go` already records for refresh, and for acquisition it
  is the sharper one: it is exactly the fresh-machine story the feature exists for. The
  writeback channel that would fix it is named there and is not built.
- **It captures the exchange it can see.** A guest reaching a token endpoint Boks is not
  intercepting keeps what it gets. That makes the recorded endpoint host load-bearing, which
  is why it was read off the binary — and why the correction from `console.anthropic.com` to
  `platform.claude.com` is a security fix rather than a tidy-up.
...
| Linux | Ubuntu 24.04+, x86_64/aarch64, KVM required, user in `kvm` group, nested virt if in a VM | Supported, same requirements | P0 | partial | `doctor` checks these. A VM has been booted on Linux since 2026-08-15, in WSL2 rather than on bare metal, and sandbox creation still needs more privilege than an ordinary user has |
| Windows | Windows 11 x86_64, Hypervisor Platform enabled, **Docker Desktop not required** | Same shape, natively: `boks run` on Windows, no WSL2 required | P2 | partial | The glue upstream libkrun lacked — a `src/vmm/windows` and a WHP `device_manager` — is written in [`packaging/libkrun-windows`](../packaging/libkrun-windows/), and on 2026-08-14 `boks run --net nat` ran a Linux container in a microVM on real Windows 11 hardware with the guest attached to Boks' own link socket and the policy engine allowing one destination and refusing another; on 2026-08-15 the same probe returned **HTTP 200** through Boks' gvisor stack, from an unelevated shell, with workspace write-through, persistence, SMP and teardown all checked ([verification.md](verification.md)). Still `partial` rather than `done`: **x86-64 only** — there is no `krun.dll` or `mkfs.erofs.exe` for ARM64 Windows — and it rests on 48 carried patches against libkrun, containerd and nerdbox, none upstream. WSL2 remains a supported alternative rather than the only route. For contrast, sbx uses the same user-mode Hypervisor Platform but loads **`sailor.dll`, Docker's own closed-source VMM**, on Windows and macOS alike — no libkrun in either, so there was never a port to copy. See [windows.md](windows.md) |
| Windows: elevation | **None.** Installs per-user into `%LOCALAPPDATA%`, ships no driver, registers no service, and `sbx diagnose` passes eight user-level checks without admin rights (verified from the shipped binaries) | Patched, unverified | P2 | partial | **Was a real gap, measured 2026-08-14, and it is containerd's rather than Boks'.** `core/runtime/v2/bundle.go:103` does `os.Symlink` unconditionally in `NewBundle` for every task bundle, and unprivileged Windows will not create a symlink without `SeCreateSymbolicLinkPrivilege` — Go passes `SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE`, which Windows honours only under Developer Mode. It was always the **containerd daemon** that needed elevation, because the daemon creates task bundles; `boks create`, `ls`, `inspect` and `rm` never did, and neither did pulling an image. `packaging/containerd-windows/patches/0006` makes that link a **junction** — creatable unprivileged, as `mklink /J` on the test machine shows — with the symlink kept as a fallback. **Nobody has run it on Windows**, which is why this is `partial`. Mechanism and evidence in `packaging/containerd-windows/README.md` |
| Windows: workspace path | `C:\Users\dag\src\foo` → `/c/Users/dag/src/foo` (verified) | Same convention, implemented in `internal/workspace/guestpath.go` | P2 | partial | Exact-path preservation is impossible on Windows; both products translate, and translate identically. The mapping and its refusals — UNC, device and drive-relative paths — are unit-tested on Linux by passing Windows paths as strings. **Never executed on Windows.** A microVM now runs there, but nothing has yet asked it to map a Windows host path into a guest, because that request comes from `boks run` and `boks run` does not start on Windows. So this is a decision made and written down rather than a feature that works. See [windows.md](windows.md) section 4 |
| Windows: policy enforcement | **Full parity with Linux/macOS** — `forward`, `forward-bypass` and `transparent` all present in the Windows binary; UDP/ICMP blocked, no platform caveat documented | Same. A VMM now exists; the device it would enforce at has never carried a frame on Windows | P2 | none | Enforcement is in Docker's userspace netstack, not in any Windows kernel component; their installer ships no driver and needs no admin rights. Boks' equivalent is `internal/network` talking to libkrun's `virtio-net`, and on Windows that pairing is compiled and never run |
...
*(Moved here from `architecture.md`, which is published on the documentation site; these
are notes about another vendor's product and belong with the rest of the parity material.)*
