...
| Windows: elevation | **None.** Installs per-user into `%LOCALAPPDATA%`, ships no driver, registers no service, and `sbx diagnose` passes eight user-level checks without admin rights (verified from the shipped binaries) | Not achieved today | P2 | none | **This is a real gap, measured 2026-08-14, and it is containerd's rather than Boks'.** `core/runtime/v2/bundle.go:103` does `os.Symlink` unconditionally in `NewBundle` for every task bundle, and unprivileged Windows will not create a symlink without `SeCreateSymbolicLinkPrivilege` — Go passes `SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE`, which Windows honours only under Developer Mode. Pulling and unpacking an image works fine as an ordinary user; creating a task needs an elevated daemon or machine-wide Developer Mode. The junction workaround used for the root-ACL blocker does not apply, because the link must live inside a `b.Path` that must not pre-exist. Options and their costs in `packaging/containerd-windows/README.md` |
