...
| Hypervisor per platform | Apple Virtualization Framework on Apple silicon; KVM on Linux | libkrun: Hypervisor.framework on macOS, KVM on Linux, and the Windows Hypervisor Platform from this project's patch series | P0 | partial | macOS path verified; Linux/KVM path still unexercised; the WHP backend booted a guest and ran a container on 2026-08-14, under `ctr` rather than Boks |
...
| Windows | Windows 11 x86_64, Hypervisor Platform enabled, **Docker Desktop not required** | Same shape; blocked on `virtio-net`, and no longer on the VMM. Interim answer: run Boks **inside WSL2** | P2 | partial | The glue upstream libkrun lacked — a `src/vmm/windows` and a WHP `device_manager` — is written in [`packaging/libkrun-windows`](../packaging/libkrun-windows/), and on 2026-08-14 the resulting `krun.dll` ran a Linux container on real Windows 11 hardware under containerd and a patched nerdbox shim ([verification.md](verification.md)). `virtio-net` has a Windows backend in that series too, and **no Ethernet frame has crossed it**, which is why the status is `partial` and not `done`: Boks' enforcement is that device, so `boks run` still refuses on Windows. For contrast, sbx uses the same user-mode Hypervisor Platform but loads **`sailor.dll`, Docker's own closed-source VMM**, on Windows and macOS alike — no libkrun in either, so there was never a port to copy. Note the mirror image: Docker calls its Linux build inside WSL "best-effort" and prefers native; Boks has only the WSL route. See [windows.md](windows.md) |
