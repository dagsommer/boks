...
| Windows | Windows 11 x86_64, Hypervisor Platform enabled, **Docker Desktop not required** | Same shape; blocked on libkrun having no Windows VMM backend: `krun-whp` (the WHP bindings) and `krun-devices` both compile for Windows, but `src/vmm` has only `linux/` and `macos/` modules and `device_manager` has only `kvm/` and `hvf/` — the glue that turns the bindings into a running VM is unwritten, as is virtio-net. Interim answer: run Boks **inside WSL2** | P2 | none | sbx uses the **Windows Hypervisor Platform** (user-mode), *not* HCS/LCOW — but behind nerdbox it loads **`sailor.dll`, Docker's own closed-source VMM**, on Windows and macOS alike; no libkrun ships in either. So sbx's Windows build proves the platform allows this in user mode, and nothing more — there is no port to copy. libkrun's WHP backend is upstream for 2.0 with virtio-net still absent, and its `main` does not compile for Windows at all. Note the mirror image: Docker calls its Linux build inside WSL "best-effort" and prefers native; Boks has only the WSL route. See [windows.md](windows.md) |
...
| Windows: workspace path | `C:\Users\dag\src\foo` → `/c/Users/dag/src/foo` (verified) | Same convention, implemented in `internal/workspace/guestpath.go` | P2 | partial | Exact-path preservation is impossible on Windows; both products translate, and translate identically. The mapping and its refusals — UNC, device and drive-relative paths — are unit-tested on Linux by passing Windows paths as strings. **Never executed on Windows**, and there is no VMM there to run a sandbox with, so this is a decision made and written down rather than a feature that works. See [windows.md](windows.md) section 4 |
