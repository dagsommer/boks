...
| Windows | Windows 11 x86_64, Hypervisor Platform enabled, **Docker Desktop not required** | Same shape; blocked on a VMM. Interim answer: run Boks **inside WSL2** | P2 | none | sbx uses the **Windows Hypervisor Platform** (user-mode) with its own VMM, *not* HCS/LCOW — same architecture as its Linux and macOS builds. Boks has no WHP-capable VMM. Note the mirror image: Docker calls its Linux build inside WSL "best-effort" and prefers native; Boks would have only the WSL route. See [windows.md](windows.md) |
